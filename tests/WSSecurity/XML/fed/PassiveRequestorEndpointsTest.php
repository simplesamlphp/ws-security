...
/**
 * Tests for fed:PassiveRequestorEndpoints.
 *
 * @covers \SimpleSAML\WSSecurity\XML\fed\PassiveRequestorEndpoints
 * @covers \SimpleSAML\WSSecurity\XML\fed\AbstractEndpointType
 * @covers \SimpleSAML\WSSecurity\XML\fed\AbstractFedElement
 * @package tvdijen/ws-security
 */
final class PassiveRequestorEndpointsTest extends TestCase
{
    ...
    {
        self::$testedClass = PassiveRequestorEndpoints::class;

        self::$schemaFile = dirname(__FILE__, 5) . '/resources/schemas/ws-federation.xsd';

        self::$xmlRepresentation = DOMDocumentFactory::FromFile(
            dirname(__FILE__, 4) . '/resources/xml/fed_PassiveRequestorEndpoints.xml'
        );
    ...
    public function testMarshalling(): void
    {
        ...
        $passiveRequestorEndpoints = new PassiveRequestorEndpoints([$endpointReference]);

        $this->assertEquals(
            self::$xmlRepresentation->saveXML(self::$xmlRepresentation->documentElement),
            strval($passiveRequestorEndpoints),
        );
    }
}
